    ...
    pub fn generate_pawn_pushes(&self, board: &Board, list: &mut MoveList) {
        ...
        while pawns.0 > 0 {
            ...
            let mut to_bb: BitBoard = match side {
                Sides::WHITE => {
                    Self::w_pawn_single_push(from_bb, empty_bb)
                        | Self::w_pawn_double_push(from_bb, empty_bb)
                }
                Sides::BLACK => {
                    Self::b_pawn_single_push(from_bb, empty_bb)
                        | Self::b_pawn_double_push(from_bb, empty_bb)
                }
                _ => panic!(),
            };
            while to_bb.0 > 0 {
