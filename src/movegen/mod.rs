...
impl MoveGenerator {
    ...
    fn white_pawn_attacks(sq: u64) -> BitBoard {
        let east_attacks =
            BitBoard(Square(sq.try_into().unwrap()).to_bb().0 << 9u64) & !BitBoard::A_FILE;
        let west_attacks =
            BitBoard(Square(sq.try_into().unwrap()).to_bb().0 << 7u64) & !BitBoard::H_FILE;
        east_attacks | west_attacks
    }

    fn black_pawn_attacks(sq: u64) -> BitBoard {
        let east_attacks =
            BitBoard(Square(sq.try_into().unwrap()).to_bb().0 >> 7u64) & !BitBoard::A_FILE;
        let west_attacks =
            BitBoard(Square(sq.try_into().unwrap()).to_bb().0 >> 9u64) & !BitBoard::H_FILE;
        east_attacks | west_attacks
    }
