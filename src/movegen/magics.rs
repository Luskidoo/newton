...
impl MoveGenerator {
    ...
    pub fn bishop_mask(sq: u8) -> BitBoard {
        ...
        let rk = sq / 8;
        let fl = sq % 8;
        for (r, f) in (rk + 1..=6).zip(fl + 1..=6) {
            result |= 1u64 << (f + r * 8);
        }
        for (r, f) in (rk + 1..=6).zip((1..fl).rev()) {
            result |= 1u64 << (f + r * 8);
        }
        for (r, f) in (1..rk).rev().zip(fl + 1..=6) {
            result |= 1u64 << (f + r * 8);
        }
        for (r, f) in (1..rk).rev().zip((1..fl).rev()) {
            result |= 1u64 << (f + r * 8);
        }
        BitBoard(result)
    }
    ...
    fn find_magic(sq: u8, m: u64, is_rook: bool) -> BitBoard {
        ...
        for i in 0..(1 << n) {
            ...
            a[i] = if is_rook {
                Self::rook_attacks(sq, b[i])
            } else {
                Self::bishop_attacks(sq, b[i])
            };
        }
