...
impl MoveGenerator {
    ...
    pub fn init_king_moves(&mut self) {
        for sq in 0..64 {
            let bb_sq = BitBoard(Square(sq.try_into().unwrap()).to_bb().0);
            let moves = BitBoard(
                (bb_sq & !BitBoard::A_FILE & BitBoard::NOT_RANK_8).0 << 7
    ...
    pub fn init_magics(&mut self, is_rook: bool) {
        ...
        for sq in 0..64 {
            ...
            let attack_boards: Vec<BitBoard> = blocker_boards
                .iter()
                .map(|blocker| {
                    if is_rook {
            ...
            for (i, &blocker) in blocker_boards.iter().enumerate() {
                ...
                if table[index] == BitBoard(0) {
                    ...
                    panic!(
                        "Attack table index not empty. Error in Magics. Square: {}, Index: {}",
                        sq, index
                    );
                }
            }
